...
Folding.prototype.fold = function(event) {
  var self = this, db = self.db, index = self.indexWith(event)
  if (index) {
    self.get(index, function(err, previousFoldingStatus) {
      ...
      var nextFoldingStatus =
        self.howToFoldEvents['$any'](
          _.clone(previousFoldingStatus), event, index
        )
      if (nextFoldingStatus && (nextFoldingStatus !== previousFoldingStatus)) {
